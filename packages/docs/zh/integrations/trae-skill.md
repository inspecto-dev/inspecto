...
Inspecto 当前通过 `AGENTS.md` 兼容模板为 Trae 提供 onboarding 接入。
...
兼容模板会让 Trae 优先使用官方的 Inspecto CLI 协议：
...
如果当前环境中的 CLI 还没有发布这些结构化命令，模板会要求 Trae 解释这个限制，并回退到 `inspecto init`。

## 说明

- 目前这是兼容模板，不是已确认的原生 Trae skill 包。
