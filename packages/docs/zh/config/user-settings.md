...
- **类型:** `"ide" | "agent" | "both"`
- **默认值:** `"both"`
- **描述:** 控制「标注模式」中哪个动作被作为主要动作并优先展示。`"agent"` 会仅显示「创建任务」按钮，这是 hands-free MCP 自动化工作流的规范设置；`"ide"` 仅显示「问 AI」，用于兼容即时的 IDE 代码内联交互流程；`"both"` 会同时显示两个按钮，并根据需要分配主次视觉。
...
- **描述:** 自定义「检查模式」下选中组件后弹出的 inspect 菜单里的 AI 快捷动作。
