...
大多数第一次接入并不需要手动编辑这份文件。请先使用 assistant-first onboarding 或 `npx @inspecto-dev/cli init`，只有在你需要调整运行时行为时，再回到这页。
...
- **类型:** `"vscode" | "cursor" | "trae" | "trae-cn" | "codebuddy" | "codebuddy-cn"`
- **默认值:** 自动探测
...
- **类型:** `"ide" | "mcp"`
- **默认值:** `"mcp"`
- **描述:** 控制「标注模式」提交时使用的通道。`"mcp"` 会显示「创建任务」按钮，这是 Agent 驱动的 MCP 工作流的规范设置，并会在侧栏展示任务 timeline；`"ide"` 会显示「问 AI」，用于兼容即时的一次性 IDE 代码内联交互流程，不会在侧栏持续追踪当前任务。

### `server.host`

- **类型:** `string`
- **默认值:** `"127.0.0.1"`
- **描述:** 控制本地 Inspecto 开发服务器绑定到哪个网络接口。常见的本地开发场景保持 `127.0.0.1` 即可。只有在你的环境需要对外访问时，才设置为 `0.0.0.0` 或其他明确的绑定地址。

### `server.publicUrl`

- **类型:** `string`
- **示例:** `"http://127.0.0.1:5678"`, `"https://my-devbox.example.com"`
- **描述:** 控制 Inspecto 注入到浏览器运行时和 MCP 发现流程中的浏览器可见基础 URL。当开发服务器监听地址和浏览器或 Agent 实际访问地址不一致时使用，例如 devbox、tunnel、容器或远程预览场景。

### `prompt.includeSnippet`

- **类型:** `boolean`
- **默认值:** `false`
- **描述:** 是否将原始代码片段以字符串形式注入到发给 AI 的 prompt 中。当使用诸如 Copilot/Cursor/Trae/CodeBuddy 等原生集成了 IDE 的 AI 工具时，将此项设置为 `false`（默认）可以为你节省大量 Token，因为 AI 本身已经可以直接读取到本地文件了。
...
### Inspect 菜单与 Workflow 自定义 (`prompts.json`)
...
- **描述:** 自定义「检查模式」下选中组件后弹出的 inspect 菜单 AI 快捷动作，也可以定义「标注模式」里的项目专属 workflow 按钮。
...
- `kind` (`"workflow"`): 将该条目标记为「标注模式」里的 workflow 按钮，而不是「检查模式」里的 prompt。
- `id` (string): 唯一 workflow 标识。
- `label` (string, 可选): 按钮显示文本，默认使用 `id`。
- `prompt` (string): 发送给 Agent 的指令。
- `confirm` (boolean, 可选): 执行前是否需要确认，默认 `false`。
- `enabled` (boolean, 可选): 设为 `false` 可隐藏该 workflow。

Workflow 按钮适合做项目级自动化，而不仅是组件级修复。例如 `Deploy Preview` 可以要求 Agent 使用自身已有的 deploy skill、MCP server 或 shell tool 部署当前分支，并通过 Inspecto session timeline 回传预览链接。
...
对于通过 MCP 发送的 `kind: "workflow"` 条目，Inspecto 还会把项目根目录、git branch、git status 等项目元信息追加到入队 session 中，帮助 Agent 更安全地执行宏观自动化任务。
...
[
  {
    ...
    "label": "中文解释",
    "aiIntent": "ask",
    "prependPrompt": "你是一个前端专家，请用中文简洁回答。"
  }
]
```
...
[
  {
    ...
    "kind": "workflow",
    "label": "Review & PR",
    "prompt": "请审查当前分支，运行合适的检查，并在确认安全后创建 PR。push 或 merge 前必须再次确认。",
    "confirm": true
  }
]
```
