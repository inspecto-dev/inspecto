# Phase Plugin: `@inspecto-dev/plugin` Build Plugin Module

`@inspecto-dev/plugin` is the core hub module connecting the front-end browser environment and the developer's local workflow. Built on the [unplugin](https://github.com/unjs/unplugin) framework, it shoulders the dual responsibilities of **build-time** code transformation and **run-time** local Dev Server communication for Inspecto.

## 1. Module Responsibilities and Positioning

If `@inspecto-dev/core` solves the problem of "clicking elements on the page", then `@inspecto-dev/plugin` solves the following key problems:
...
2. **Webpack / Rspack**: Uses `EntryPlugin` to dynamically bundle `@inspecto-dev/core` as an additional entry point, and uses `HtmlWebpackPlugin` (or Next.js's `processAssets` hook) to inject initialization code with `window.InspectoClient.mountInspector` into the HTML/output.
3. **Rollup / Esbuild**: As pure JS, lower-level bundlers lacking a unified HTML processing ecosystem, no automatic injection is performed. Developers in these environments must **manually call** `import { mountInspector } from '@inspecto-dev/core'` in their entry files to initialize it.
...
When the host build tool starts, `unplugin` spins up an independent local Node.js HTTP Server (using `portfinder` to automatically find an available port starting from `5678`). This Server acts as a middleware handling requests from the `@inspecto-dev/core` Client:
...
[IDE Extension Activation]
       ...
       ▼
[@inspecto-dev/plugin Local Server]  ←──────────────────────────────────┐
...
[@inspecto-dev/plugin Local Server]                                     │
       ...
       │ 3. Reads settings.json, parses prefer → ProviderMode            │
