...
export class InspectoUriHandler implements vscode.UriHandler {
  ...
  private matchesCurrentWorkspace(targetWorkspace: string): boolean {
    const normalizedTarget = path.resolve(targetWorkspace)
    const matchedFolder = vscode.workspace.getWorkspaceFolder(vscode.Uri.file(normalizedTarget))
    if (matchedFolder) {
      return path.resolve(matchedFolder.uri.fsPath) === normalizedTarget
    }

    const folders = vscode.workspace.workspaceFolders ?? []
    return folders.some(folder => path.resolve(folder.uri.fsPath) === normalizedTarget)
  }
}
