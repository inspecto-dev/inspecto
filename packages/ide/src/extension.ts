...
import type { IdeInfo } from '@inspecto-dev/types'
  ...
  const pushInfoWithRetry = async (attempt = 1): Promise<boolean> => {
    ...
    const info: IdeInfo = {
      ...
      providers: {
        ...
        'claude-code': {
          mode: isExtensionInstalled('anthropic.claude-code') ? 'extension' : 'cli',
          installed: targets.includes('claude-code'),
        },
        gemini: {
          mode: isExtensionInstalled('google.geminicodeassist') ? 'extension' : 'cli',
          installed: targets.includes('gemini'),
        },
        codex: {
          mode: isExtensionInstalled('openai.chatgpt') ? 'extension' : 'cli',
