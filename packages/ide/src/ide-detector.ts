...
import { getDualModeProviderCapability, type IdeType, type Provider } from '@inspecto-dev/types'
...
export function resolveAvailableTargets(ide: IdeType): Provider[] {
  ...
  if (
    claudeCapability &&
    (isExtensionInstalled(claudeCapability.extensionId) || isCliAvailable(claudeCapability.cliBin))
  )
  ...
  if (
    geminiCapability &&
    (isExtensionInstalled(geminiCapability.extensionId) || isCliAvailable(geminiCapability.cliBin))
  )
    targets.add('gemini')
  ...
  if (
    codexCapability &&
    (isExtensionInstalled(codexCapability.extensionId) || isCliAvailable(codexCapability.cliBin))
  ) {
    targets.add('codex')
  }
