...
import type { AiStrategyResult } from '@inspecto-dev/types'
...
export function ensureClipboardSuccess(result: AiStrategyResult): void {
  if (!result.success) {
    throw new RecoverableChannelError(result.error ?? 'Clipboard operation failed')
  }
}
