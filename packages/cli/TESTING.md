...
2. **Expectations:**
   ...
   - If `@inspecto/plugin` cannot be installed (e.g., registry not ready), it logs the failure but does not crash.
...
2. **Expectations:**
   ...
   - Removes `@inspecto/plugin` from `package.json` `devDependencies` (if it was installed).
