...
```
@inspecto/types          ← leaf, no internal deps
        ↓
@inspecto/unplugin       ← build-time only, Node.js
        ↓
@inspecto/core           ← browser runtime + server utils
        ↓
@inspecto/react          ← thin React wrapper (peer: react)
@inspecto/vue            ← thin Vue wrapper (peer: vue)
        ↓
@inspecto/ide                    ← IDE extension (VS Code, Cursor, Trae) (CommonJS, Node.js)
```
