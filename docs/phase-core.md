...
The front-end build plugin (like `@inspecto/plugin`) injects the `data-inspecto` attribute into component DOMs at compile time. The Core Client is responsible for parsing this attribute at runtime:
